...
\section{紧性}
     ...
     \begin{Proposition}
           $ E $ 紧的充分必要条件是任意闭集族 $ (F_{i})_{i\in\alpha} $ , 若 $ \forall \beta\in\fin\alpha $ 都有 $ \bigcap_{i\in\beta}F_{i}\neq \varnothing $ , 则有 $ \bigcap_{i\in\alpha}F_{i}\neq\varnothing $. 
     \end{Proposition}
     ...
     \begin{Proof}
          ...
          \hspace{2em}\textsl{充分性}. 若存在有限集 $ \beta\in\fin\alpha $, 使得 $ F\bigcup_{i\in\beta}O_{i} $ , ze 
          \[
               F=F\cap\left( \bigcup_{i\in\beta}O_{i} \right)=\bigcap_{i\in\beta}U_{i},
          \]
          所以 $ F $ 是紧子空间.\qed
     \end{Proof}
     ...
     \begin{Proof}
          令 $ x\in F^{c} $ , 由 $ E $ 是 Hausdorff空间, 则对任意 $  y\in F $ , 存在 $ U_{y}\in\CN{x} $, $ V_{y}\in\CN{y} $ , 其中 $ U_{y} $ , $ V_{y} $ 是开集, 使得 $ U_{y}\cap V_{y}\varnothing $ 
     \end{Proof}
