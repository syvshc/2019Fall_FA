...
\section{乘积拓扑}
	...
	设$ (E,d) $是度量空间, $ \forall A\subset E $, 映射$ x\mapsto d(x,A) $是连续的.
     ...
     \begin{Theorem}
          设 $ E_{1} $ 和 $ E_{2} $是两个紧的拓扑空间, 则乘积拓扑空间 $ E=E_{1}\times E_{2} $ 也是紧的.
     \end{Theorem}
     \begin{Proof}
          ...
          \]
          事实上由以上讨论可以知道, 当我们任取一个 $ a=(a_{1}, a_{2})\in E $, 必存在某个 $ x_{2}\in K $, 使得 $ a_{2}\in A_{x_{2}} $, 那么存在 $ x\in J_{x_{2}} $, 使得 $ a_{1}\in V_{x} $; 这里确定的 $ x\in J $, 且 $ a\in V_{x}\times W_{x} $, 由此可见,  $ E $ 被 $ (O_{i_{x}})_{i_{x}\in J} $ 覆盖, 因为 $ J $ 是有限集, 则 $ E $ 是紧的.\qed
     \end{Proof}
...
\section{赋范线性空间}
	...
	\textsl{充分性}. 用反证法, 假设$ E $是无限维的, 则可以取一单位向量$ x_1 $, 并且记$ F_1=\mathrm{span}\{x_1\}=\{ \lambda_1x_1 : \lambda_1\in\K \} $. 由$ \dim F_1=1<\infty $可知$ F_1 $完备, 从而$ F_1 $是$ E $的闭线性子空间. 故有引理\ref{lem:Riesz前引理}可知
	\[
