\section{第6章习题}
	...
	\begin{Proof}
		...
		\]
		为一个无内点的闭集, 矛盾.  而任意 $ f: \R\to\R $ 的连续点集 $ \cont{f} $ 是一个 \Gd 集, 故不存在函数 $ f:\R\to\R $ 使得 $ \cont(f)=\Q $.
	...
	\end{enumerate}
	\begin{Proof}
	(a)$ \Rightarrow $(b) : 由$ (u_n(x))_{n\geqslant 1} $对任意$ x\in E $收敛可知$ (u_n(a))_{n\geqslant 1} $收敛, 且可知其对任意$ x\in E $有界, 从而由共鸣定理可知$ \sup\limits_{n\geqslant 1}\norm{u_n}<\infty $.
	
	(b)$ \Rightarrow $(a) : 由$ (u_n)_{n\geqslant 1} $有界, 可知存在$ M>0 $使得$ \sup\limits_{n\geqslant 1}\norm{u_n}\leqslant M $. 则
	\[ \forall x\in E\,\exists a\in\Span A\,(\norm{x-a}<\varepsilon/4M).
	\]
	且$ \exists n_0\in\N $使得$ m,n\geqslant n_0 $时有$ \norm{u_n(a)-u_m(a)}<\varepsilon/2 $(此因$ \Span A $中的元素是$ A $中元素的有限线性组合), 那么
	\[
	\begin{aligned}
	...
	\norm{y-u(x_0)}<q,
	\]
	取$ y_1=\frac{1}{q}(y-u(x_0)) $, 则$ y_1\in B_F $, 则存在$ x_1\in B_E $使得
	\[
	\norm{y_1-u(x_1)}<q,
	\]
	...
	\]
	故$ v(\bar{B}_{E/\ker u})\subset\baro{v(B_{E/\ker u})}=\bar{B}_F $. 即$ \norm{v}<1 $. 而$ v^{-1}(B_F)=B_{E/\ker u} $, 同理$ \norm{v^{-1}}\leqslant 1 $. 故
	\[
	...
	\begin{Proof}
		...
		\[
			\frac{r}{2}\bar{B}_{F+G}\subset rB_{F+G}\subset u(B_{F\times G}),
		\]
		取 $ c=2/r $, 有 $ \frac{1}{c}\bar{B}_{F+G}\subset u(B_{F\times G}) $, 从而 $ \forall x\in F+G, \frac{x}{c\norm{x}}\in\frac{1}{c}\bar{B}_{F+G} $, 则 $ \exists (f_{0}, g_{0})\in B_{F\times G} $ 使得
		\[
...
\section{第8章习题}
	...
	(3a) \textsl{充分性}. 由题设可知存在$ x_0\in E\sm H $使得$ f(x_0)\ne0 $且$ H=\ker f $, 则任取$ e\in E $都有
	\[
	...
	\[
	\forall n\geqslant 1\,\left(f(e_{2n-1})=0\land f\left(e_{2n-1}+\frac{e_{2n}}{2^n}\right)=0\right)
	\]
	...
	&=\lim_{k\to\infty}\sum_{n=1}^k(2^nx_{2n}e_{2n-1}+x_{2n}e_{2n})+(x_{2n-1}-2^nx_{2n})e_{2n-1}\in\baro{A_0+B}.
	\end{aligned}
	\]
	)
	...
	\textbf{习题8.15(a)}\ [习题课]\ \ 本习题的目的是刻画连续线性泛函的 Hahn-Banach 延拓的唯一性问题. 设 $ E $ 是一个维数至少为 2 的赋范空间, 证明: $ E $ 的线性子空间 $ F $ 上的每个连续线性泛函有唯一的保范延拓的充分必要条件是 $ \Star{E} $ 是严格凸的.
	...
	\begin{Proof}
		...
		(i) 因为
		\[
			\frac{1}{n}\sum_{k=1}^{n}x_{k}\leqslant\frac{1}{n}\sum_{k=1}^{n}\sup_{i\geqslant k}x_{i},
		\]
...
\section{第9章习题}
	...
	\begin{Proof}
		...
		\[
			\forall f\in\Star{E}\,\forall \varepsilon>0\,\exists N\in\N\,\forall n>N\,(\norm{f-f_{n}}<\varepsilon),
		\]
...
\section{第 11 章习题}
	...
	\begin{Proof}
		...
		(4) 只需证明 $ T(B_{E}) $ 相对紧, 则只需证明 $ T(B_{E}) $ 中任意无穷序列都有收敛于 $ F $ 中元素的子列. 设 $ (y_{n})_{n\geqslant1} $ 是 $ T(B_{E}) $ 中的一个无穷序列, 则存在 $ (x_{n})_{n\geqslant1}\subset B_{E} $ 使得 $ y_{n}=T(x_{n}), \forall n\geqslant1 $. 由习题 9.4 知 $ (x_{n})_{n\geqslant1} $ 有收敛于某个 $ x\in E $ 的子列 $ (x_{n_{k}})_{k\geqslant1} $, 由题知 $ T(x_{n_{k}}) $ 依范数收敛于 $ T(x) $, 即 $ (y_{n_{k}}) $ 依范数收敛于 $ T(x)\in F $, 故 $ T(B_{E}) $ 相对紧.
...
\section{Problem Set系列}
	...
	\begin{Definition}[严格凸]\index{Y!严格凸}\label{def:严格凸}
		设 $ E $  是一个赋范空间, 若其单位球面 $ S_{E} $ 上每个点是 $ \bar{B}_{E} $ 的端点, 则称 $ E $ 是\textbf{严格凸}的.(即任取 $ x\in S_{E} $, 不存在 $ \bar{B}_{E} $ 中相异的两点 $ x_{1}, x_{2} $, 使得 $ x=(x_{1}+x_{2})/2 $.)
	\end{Definition}
	\begin{ExtraExample}
