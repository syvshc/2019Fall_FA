...
\section{第16周习题课(2019年12月17日)}
	...
	\textbf{习题9.2(a)}\ [习题课]\ \ 设$ E $是Banach空间, $ B\subset\Star{E} $, 证明: $ B $是相对$ \Star{w} $--紧的当且仅当$ B $是有界的.
	\begin{Proof}
	\textsl{必要性}. 设$ B $相对$ \Star{w} $--紧, 即$ \bar{B}^{\Star{w}} $是$ \Star{w} $--紧的. 任取$ x\in E $, 都有$ \hat{x} $是连续的, 于是$ \hat{x}(\bar{B}^{\Star{w}}) $是$ \K $中的紧集, 即是有界闭集, 故$ \{ f(x) \}_{f\in \bar{B}^{\Star{w}}} $有界. 由一致有界原理可知$ \{ f\in\bar{B} \} $有界, 即$ B $是有界的.
	...
	\textsl{必要性}. 注意到$ \tnorm{\widehat{u(x)}}=\norm{u(x)} $, 此因等距嵌入, 则
	\[
