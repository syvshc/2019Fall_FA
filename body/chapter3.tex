...
\section{正则Borel测度和Riesz表示定理}
	...
	以 $ C_c(X) $ 记具有紧支撑的连续函数构成的线性空间, 由 Urysohn引理可知对于任意的 $ K\subset O $, 总存在 $ f\in C_c(X) $ 使得 $ K\prec f\prec O $.

	\subsection{正线性泛函的表示定理}

	\begin{Definition}[正线性泛函]\index{Z!正线性泛函}
	...
	\begin{Definition}[局部有限测度]\index{J!局部有限测度}
	...
	\begin{Proposition}[Riesz]\index{R!Radon测度}
	...
	\begin{Definition}[正则测度]\index{Z!正则测度}
