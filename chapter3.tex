...
\section{自反性}
	...
	\begin{Proof}
		...
		\[
		\norm{\sum_{k=1}^nx_ke_k-x}_p=\norm{\sum_{k\geqslant n+1}x_ke_k}_p=\sum_{k\geqslant n+1}\abs{x_n}^p\to 0,\qquad(n\to\infty)
		\]
		...
		\[
		\begin{aligned}
		\abs{f\left(\sum_{k\geqslant 1}x_ke_k\right)}&=\abs{\sum_{k=1}^nx_kf(e_k)}=\sum_{k=1}^n\abs{f(e_k)}^q\\
		&\leqslant\norm{f}\cdot\norm{\sum_{k\geqslant 1}x_ke_k}_p=\norm{f}\left(\sum_{k=1}^n(\abs{f(e_k)}^{q-1})^p\right)^{1/p}=\norm{f}\left(\sum_{k=1}^n\abs{f(e_k)}^q\right)^{1/p},
		\end{aligned}
		\]
