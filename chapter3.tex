...
\section{半范数}
	\begin{Definition}[半范数]\label{def:半范数}\index{B!半范数}
		设 $ E $ 是数域\K 上的线性空间, 设 $ E $ 上的泛函 $ p : E\to \R $ 满足
		\begin{enumerate}[(1)]
...
\section{Hahn-Banach定理}
	...
	\begin{Proof}
		...
		\]
		故 $ \forall x\in E\,(|\tilde{f}(x)|\leqslant p(x)) $.
	...
	从而
	\[
	f(a)\leqslant f(b)+1-p_C(a-b+x_0)\leqslant f(b),
	\]
	...
	\]
	令$ r=\min\{ r_{x_1}/2,r_{x_2}/2,\dots,r_{x_n}/2 \} $, 并令$ \tilde{A}=A+B(0,x) $, 则$ \tilde{A} $是凸集且是开集, 往证$ \tilde{A}\subset B^c $. 取$ \tilde{x}\in\tilde{A} $, $ \tilde{x}=x+y $, 其中$ x\in A,\ y\in B(0,r) $, 则对$ x $, 存在$ x_k $使得$ x\in B(x_k,r_{x_k}/2)=x_k+B(0,r_{x_k}/2) $. 从而
	\[
	...
	令$ \mathcal{C} $是以下开凸集全体: $ 0\notin C $, $ A-B\subset C $. 并在$ \mathcal C $上以包含关系作为偏序关系, 则$ (\mathcal C,\leqslant) $是一个偏序集. 取$ \mathcal C_0 $是$ \mathcal C $的全序子集, 作$ \tilde{C}=\bigcup\mathcal C_0 $, 则$ \tilde{C} $是$ \mathcal C $的上界. 由Zorn引理可知$ \mathcal C $有极大元$ C $.
...
\section{弱拓扑与弱*拓扑}	
	...
	设$ E $是赋范空间, $ (x_n)_{n\geqslant 1} $是$ E $中序列. 弱存在$ x\in E $使得$ \forall f\in\Star{E} $都有
	\[
