...
\section{Fourier分析}
		...
		\begin{Definition}[Fourier系数, Fourier变换]\label{def:Fourier系数, Fourier变换}\index{F!Fourier系数, Fourier变换}
			设 $ f\in L_{2\pi}^{1} $, 称
			\[
				\hat{f}(n)=\frac{1}{2\pi}\int_{0}^{2\pi}f(\theta)\exp(-\imag n\theta)\diff\theta,\quad n\in\Z
			\]
			为函数  $ f $ 的 $ n $ 阶\textbf{Fourier系数}, 并记 $ f $ 的所有Fourier系数构成的序列为 $ \mathcal{F}(f)=(\hat{f}(n))_{n\in\Z} $, 称其为 $ f $ 的\textbf{Fourier变换}, 有时也以 $ \hat{f} $ 记 $ \mathcal{F}(f) $. 
		\end{Definition}
		\begin{Theorem}
			~
			\begin{enumerate}[(1)]
				\item 设 $ 0<p<\infty $, 则 $ \mathcal{P} $ 在 $ C_{2\pi} $ 与 $ L_{2\pi}^{p} $ 中都稠密.
				\item $ \mathcal{F} : L_{2\pi}^{1}\to c_{0}(\Z) $ 是范数为 $ 1 $ 的单射, 其中
				\[
		...
		\begin{Proof}
			...
			\[
				\norm{\mathcal{F}(f)}_{\infty}=\sup_{n\in\Z}\abs{\hat{f}(n)}=\sup_{n\in\Z}\abs{\int_{0}^{2\pi}f(\theta)\exp(-\imag n\theta)\frac{\diff\theta}{2\pi}}\leqslant\norm{f}_{L_{2\pi}^{p}}
			\]
			知 $ \norm{\mathcal{F}}\leqslant1 $, 取 $ e_{n}=\exp(-\imag n\theta) $ 后由 $ \norm{\mathcal{F}(e_{n})}=1 $ 知 $ \norm{\mathcal{F}}=1 $. 
			...
			\[
				\abs{\hat{f}(m)}=\abs{\hat{f}(m)-\hat{g}(m)}\leqslant\norm{\mathcal{F}(f)-\mathcal{F}(g)}_{\infty}\leqslant\norm{f-g}_{L_{2\pi}^{1}}<\varepsilon,
			\]
			故 $ (\hat{f}(n))_{n\in\Z}\in c_{0}(\Z) $.
			
			最后说明 $ \mathcal{F} $ 是单射, 因而 $ \ker \mathcal{F}=\{ 0 \} $, 而:
			\[
				\begin{aligned}
					\mathcal{F}(f)=0 & \Longleftrightarrow \int_{0}^{2\pi}f(\theta)\exp(-\imag n\theta)\diff\theta=0, \quad\forall n\in\Z\\
		...
		\begin{Remark}
			定理中\,(2)\,的一部分结论: $ \mathcal{F}(f)\in c_{0}(\Z) $ 即为所谓 Riemann-Lebesgue引理, 且由\,(3)\,中结论与 Parsevaal 等式, $\forall f\in L_{2\pi}^{2}$, 有
			\[
