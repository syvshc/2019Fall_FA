...
\section{正交基}
	...
	常见空间上的规范正交基:
	\begin{enumerate}[(1)]
	\item 在$ \K^n $上, 取$ \{ e_1,\cdots,e_n \} $, 其中$ e_i=[\delta_{1,i},\delta_{2,i},\cdots,\delta_{n,i}]^\mathrm T $, 它是$ \K^n $的经典基(或称标准基/自然基).
	\item 在$ \ell_2 $上, 取$ (e_n)_{n\geqslant 1} $, 其中$ e_n=(\delta_{1,n},\delta_{2,n},\cdots) $, 则$ (e_n)_{n\geqslant 1} $是$ \ell_2 $的一组规范正交基.
	...
	下面给出投影算子的代数表达: 设$ H $是Hilbert空间, $ \{ e_1,\cdots,e_n \} $是$ H $的规范正交集, 令$ E=\Span\{ e_1,e_2,\cdots,e_n \} $, 则$ \forall x\in H $, 有
	\[
	P_E(x)=\sum_{i=1}^n\lrangle{x,e_i}e_i,\qquad\text{且}\qquad \norm{x}^2=\norm{x-P_E(x)}^2+\sum_{i=1}^n\abs{\lrangle{x,e_i}}^2.
	\]
	\end{Proposition}
	\begin{Proof}
	令$ y=\sum\limits_{i=1}^n\lrangle{x,e_i}e_i $, 注意到$ P_E(x) $是满足$ x-P_E(x)\bot E $的唯一向量, 故只需验证$ \lrangle{x-P_E(x),e_i}=0 $即可. 而对$ i=1,2,\cdots,n $, 有
	\[
	...
	\begin{Theorem}
	设$ H $是Hilbert空间, $ (e_n)_{n\geqslant 1} $是规范蒸饺序列, 则下列叙述等价:
	\begin{enumerate}[(1)]
	\item $ (e_n)_{n\geqslant 1} $是完全的.
	\item $ \forall x\in H\,\exists!(x_n)_{n\geqslant 1} $使得$ \sum\limits_{n\geqslant 1}x_ne_n $收敛到$ x $.
	\end{enumerate}
	\end{Theorem}
	\begin{Proof}
	...
	\end{Theorem}
	\begin{Proof}
	因为$ x=\sum_{n\geqslant 1}\lrangle{x,e_n}e_n $, 故有$ \norm{x}^2=\sum_{n\geqslant 1}\abs{\lrangle{x,e_n}}^2 $, 而$ \forall x,y\in H $, 有
	\[
	...
	\]
	从而$ A $可逆, 于是$ [f_1,f_2,\cdots,f_n]^\mathrm T=A^{-1}[e_1,e_2,\cdots,e_n]^\mathrm T $, 也即
	\[
	\baro{\Span(e_k)_{1\leqslant k\leqslant n}}=\baro{\Span(f_k)_{1\leqslant k\leqslant n}}
	\]
	\qed
	...
	\end{Theorem}
	\begin{Proof}
	设$ \mathcal F $是$ H $中所有非空规范正交集全体, 以包含关系为偏序后有$ (\mathcal F,\leqslant) $是一个偏序集. 任取$ \mathcal F $中的全序子集$ \mathcal G $, 那么$ \bigcup\mathcal G $是$ \mathcal G $的一个上界, 由Zorn引理可知$ \mathcal F $有极大元$ M $.
	...
	\end{Corollary}
	\begin{Proof}
	$ \forall x\in H $, $ x=\sum\limits_{n\geqslant 1}\lrangle{x,e_n}e_n $, 由Parseval不等式可知
	\[
	\norm{x}=\left( \sum_{n\geqslant 1}\abs{\lrangle{x,e_n}}^2 \right)^{1/2},
	\]
	定义$ \varPhi : H\to \ell 2 $, $ x\mapsto(\lrangle{x,e_1},\lrangle{x,e_2},\cdots) $, 则$ \norm{\varPhi(x)}=\norm{x} $, 因此$ \varPhi $是一个线性等距单射, 下面证明$ \varPhi $是满的. $ \forall(x_n)_{n\geqslant 1}\in\ell_2 $, 令$ x=\sum_{n\geqslant 1}x_ne_n $, 则
	\[
	\norm{x}=\left( \sum_{n\geqslant 1}\abs{x_n}^2 \right)^{1/2}<\infty,
	\]
	即存在$ x\in H $使得$ \varPhi(x)=(x_n)_{n\geqslant 1} $, 从而$ \varPhi $是满的.\qed
	\end{Proof}
	\begin{Remark}
	Hilbert正交基与Hamel基不同, 在有限维情形下, Hilbert正交基一定是Hamel基, 而Hamel基也可以由Gram-Schmidt正交化得到Hilbert正交基. 但在无限维情形下, Hilbert基未必是Hamel基, 例如在$ \ell_2 $上取$ (e_n)_{n\geqslant 1} $, 其中$ e_n=(\delta_{1,n},\delta_{2,n},\cdots) $, 考虑$ \sum_{n\geqslant 1}e_n/n\in\ell_2 $, 它不能被有限个$ e_n $生成, 从而$ (e_n)_{n\geqslant 1} $不是Hamel基. 反之, Hamel基也未必能生成Hilbert正交基.
	\end{Remark}